...
?> </select> 
 ...
 Select Clamping Team: 
 <select name="street" class="components" id="street" >
<?
$query_courses = "select * from ".DATABASE.".c_ order by street_name ASC";
$result_crs = run_query($query_courses);
while($ents = get_row_data($result_crs))
{
	$code = $ents['street_id'];
...
?> </select> 
   ...
   <input style="background-color:#900; color:#FFF; font-weight:bold;" type="submit" name="add_inspector" id="add_inspector" value="Update Clamp-Team Region" />
