...
include("rate_module/rate_library.php");
...
function logAction($case_name, $session_id, $mf_id){
    date_default_timezone_set('Africa/Nairobi');
    $timestamp = date('Y-m-d H:i:s');
    $query = "INSERT INTO audit_trail(session_id, mf_id, case_name, datetime) VALUES ('".$session_id."', '".$mf_id."', '".$case_name."', '".$timestamp."')";
    if(run_query($query)){
        return true;
    }else{
        return false;
...
function checkForExistingEntry($tablename, $column_name, $id){
  $check_query = "SELECT $column_name FROM $tablename WHERE $column_name = '".$id."'";
