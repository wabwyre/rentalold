<?
	class ParkingSession{
		...
		function ParkingSession($reference_id,$car_no,$customer_phone,$customer_email,$parkingtype,$today,$start_date,$end_date,$agent_id, $agent_trans_id,
								$zone,$amt,$street,$lot,$cartype,$months)
		{
			$this->car_no = $car_no;
			$this->today = $today;
		...
		{ 
			...
			$email = "omodhog@hausraum.com";
	
			$formdate = date("d-m-Y",$this->today);
			  $message  = "Thank you for purchasing a Parking Ticket for [".$this->car_no."] on [".$formdate."]:Your 
			  			   Ticket Number is: " .$this->receipt;
			  
			  $sender_id = "CityCouncil EBPPP";
			  
			  // open the connection to majorcalendar      
			  $host = "www.majorcalendar.com";
			  $path = "http://www.majorcalendar.com/emailapi/send_email.php";
			  $post_string = "action=send_email&email=".$this->customer_email."&message=".$message."&sender_email=".$email."&sender_name=".$sender_id;       
