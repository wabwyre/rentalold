<?php
	//require('Connections/pg_connect.php');
...
//	define("HOST", "192.168.100.13");
//	define("DB", "ebppp");
//	define("USERNAME", "Isaac");
//	define("PASSWORD", "atuta");

	//echo HOST;
//////////////////////// db connections and disconnection //////////////////////////////////////

	function connectToPostgresEbppp(){
		...
		if(!$dbcon){
                    ...
                    die('Unable to connect to .13 ebppp');

                    }

	}

	function connectToPostgresMenu(){
		$menucon=pg_connect("host=192.168.100.13  port=5432 dbname=menu user=Isaac password=atuta");
		if(!$menucon){die('Unable to connect to .13 menu');}
...
/////////////***************************//////////////////////////////////////////////////////////////
	...
	{
		$dbcon=connectToPostgresEbppp();
// traceActivity("...entered getKeyWordOfOptionCode.......optioncode: ".$optionCode);
		...
		$result=pg_fetch_array($query_result);
              ...
              //  traceActivity("...entered getKeyWordOfOptionCode.......keyword: ".$result['keyword']);
	...
	{
                         traceActivity("... entered getServiceIDOfOptionCode vr 1.1...".$optionCode);

					//open connection to menu
					connectToPostgresEbppp();

                              traceActivity("...hhhhhhhhh.......");
					$kw=getKeyWordOfOptionCode($optionCode);
                             traceActivity("... entered getServiceIDOfOptionCode vr 1.1... got keyword as: ".$kw);

					$query=pg_query("SELECT service_id FROM options_junction WHERE keyword='$kw'");
					$array=pg_fetch_array($query);

					$result=$array['service_id'];

                              //   traceActivity("... entered getServiceIDOfOptionCode vr 1.1... got service id as: ".$result);
	...
	function getServiceTypeOfServiceID($service_id)
		{
			...
			$query = pg_query("SELECT * FROM services where service_id ='$service_id'");
			$array = pg_fetch_array($query);

			$service_type_id = $array["service_type_id"];
