<?
	...
	*************************************************************************************************************/
	...
	if($_POST['TRANSACTIONCODE'])
	{
		...
		traceActivity("...Agent[$agent_code]POST=$trans_code=$service_code=$user_account=$description=$amount=$date_logged=SWC[$swc_version]");		
	}
	else
	{
...
/************************************************************************************************************/
	...
	elseif($trans_code == "PAYBILL")
	{
		...
		{
			//$receipt = processBillPayment($bill_id,$paid_amt,$agent_id,$agent_trans_id,$service_id,$service_type);
...
/*if(validateAgent($_POST['agent_id']))
{
	...
	{
		$report = $nature." ACCOUNT ".$acc_id." IS ".$status." Contact Customer Care in-case of any questions...";
		$client = new SoapClient("http://192.168.100.13/bibi/soap/ccn_ebppp.wsdl");
			//echo "<hr>";
			$return_string = array('uspCode' => "CCN", 
								   'uspPassword' => "123456",
	...
	function returnBillAmount($trans_id,$nature,$reference_id,$bill_id,$status)
	{
		$report = "CCN:Bill[".$bill_id."] Current Balance is Ksh. ".getBillBalance($bill_id);
		$client = new SoapClient("http://192.168.100.13/bibi/soap/ccn_ebppp.wsdl");
			//echo "<hr>";
			$return_string = array('uspCode' => "CCN", 
								   'uspPassword' => "123456",
	...
	{
		$report = "CCN:Bill[".$bill_id."] Doesn't exist in our system...";
		$client = new SoapClient("http://192.168.100.13/bibi/soap/ccn_ebppp.wsdl");
			//echo "<hr>";
			$return_string = array('uspCode' => "CCN", 
								   'uspPassword' => "123456",
	...
	{
		$report = "CCN:BUSINESS[ ".$biz_id." - ".getBusinessName($biz_id)." ]. YOUR TOTAL BILL IS: Ksh. " .getBusinessRate($biz_id);
		$client = new SoapClient("http://192.168.100.13/bibi/soap/ccn_ebppp.wsdl");
			//echo "<hr>";
			$return_string = array('uspCode' => "CCN", 
								   'uspPassword' => "123456",
	...
	function returnBillForLandRate($trans_id,$trans_type,$reference_id,$rate_id)
	{
		...
		formatMoney(getLandRateBalance($rate_id),2)." as at ".date("Y-m-d H:i").". ANNUAL RATE: Ksh. ".formatMoney(getLandRate($rate_id),2);
		$client = new SoapClient("http://192.168.100.13/bibi/soap/ccn_ebppp.wsdl");
			//echo "<hr>";
			$return_string = array('uspCode' => "CCN", 
								   'uspPassword' => "123456",
	...
	function processBillPayment($bill_id,$paid_amt,$agent_id,$agent_trans_id,$service_id,$service_type)
	{
		$today = date("Y-m-d");
		...
		$ins_sql = "INSERT INTO ".DATABASE.".transactions(transaction_id, receiptnumber, agent_id, agent_trans_id, service_id,service_type_id, cash_paid,
					   ...
					   VALUES ('$trans_id','$receipt','$agent_id','$agent_trans_id','$service_id','$service_type','$paid_amt','$today','FALSE')";
		echo "<hr>".$ins_sql;			
		run_query($ins_sql);
