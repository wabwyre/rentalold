...
class pgBackupRestore
{
   ...
   function next_record()
   {
      ...
      $this->Record = get_row_data($this->Query_ID, $this->Row++);
   ...
   function Backup($uiFilename = NULL)
   {
      ...
      // var_dump(empty($this->Tables));exit;
      ...
      foreach($this->Tables as $Table)
      {
         ...
         $strSQL = '';
         $strSQL .= "DROP TABLE IF EXISTS ${Table} CASCADE;\n\n CREATE TABLE ${Table} (";
         ...
         if ($_sequences)
         {
            foreach($_sequences as $_seq_name)
            {
               ...
               $this->writeSQL("DROP SEQUENCE IF EXISTS $_seq_name CASCADE;\nCREATE SEQUENCE ${_seq_name} INCREMENT ${_incrementby} MINVALUE ${_minvalue} ".
         ...
         {       
            ...
            while ( $this->next_record() )
            {
               ...
               if (preg_match("^CREATE UNIQUE INDEX", $_pggetindexdef))
               {
      ...
      $this->commentSQL("The 'Views'\n");

      //---[GENERATE THE VIEWS]
      $SQL = "SELECT definition, viewname FROM pg_views WHERE schemaname NOT IN('information_schema', 'pg_catalog');";
      $this->query($SQL);
      while ($this->next_record()) {
         $definition = $this->get('definition');
         $viewname = $this->get('viewname');
         $this->writeSQL("DROP VIEW IF EXISTS allocated_indices;\nCREATE OR REPLACE VIEW $viewname AS\n $definition");
      }
   ...
   function Restore($uiFilename = NULL)
   {
      ...
      // var_dump($_readSQL);exit;
      while ( $_readSQL = fgets($_fpSQL) )
      { 
         $_CurrentLine++;

         if (preg_match("^-", $_readSQL)) continue; // Don't bother about comments
         $this->query(utf8_encode($_readSQL));

         $error = '';
         $error .= pg_last_error();
         $_SESSION['backup_restore'] = $error;

         echo $this->GotSQLerror;
         if ($this->GotSQLerror)
   ...
   function recordBackup($file_path, $file_name){
      $query = "INSERT INTO backup_log(filename, backup_user, file_path, stamp) 
      VALUES('".$file_name."', '".$_SESSION['mf_id']."', '".$file_path."', '".time()."')";
      //var_dump($query);exit;
   ...
   function getRestoreFile(){
      ...
      $temp = explode(".", $_FILES["restoration_file"]["name"]);
      $extension = end($temp);
      $file_name = 'Restore_'.uniqid().'.'.$extension;
      if (($_FILES["restoration_file"]["size"] < 5000000)
        && in_array($extension, $allowedExts)) {
            if ($_FILES["restoration_file"]["error"] > 0) {
               "Return Code: " . $_FILES["restoration_file"]["error"] . "<br>";
            } else {
               $target_path = "assets/db_backups/".$file_name;
               if(move_uploaded_file($_FILES["restoration_file"]["tmp_name"], $target_path)){
                  return $file_name;
               }
            }
      }
   ...
   function getBackupUser($user){
      $query = "SELECT CONCAT(m.surname,' ',m.firstname,' ',m.middlename) AS user_name FROM backup_log b
               LEFT JOIN masterfile m ON m.mf_id = b.backup_user
               WHERE b.backup_user ='".$user."'";
       $result = run_query($query);
